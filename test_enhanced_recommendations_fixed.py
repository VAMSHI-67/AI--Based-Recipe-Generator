...
def test_enhanced_recommendations():
    ...
    print("[OK] Loaded {} recipes".format(len(recipes_df)))
    ...
    print("[OK] Built TF-IDF model with {} features".format(
        len(recommender.tfidf_vectorizer.get_feature_names_out())
    ))
    ...
    print("\nFound {} recommendations:".format(len(recommendations)))
    print("\nEXPECTED BEHAVIOR:")
    ...
    if len(recommendations) == 0:
        ...
        if hasattr(recommendations, '_fallback_notification'):
            print("  Reason: {}".format(recommendations._fallback_notification))
    else:
        for idx, (_, recipe) in enumerate(recommendations.iterrows(), 1):
            ...
            print("\n  {}. {} {}".format(idx, recipe['recipe_name'], status))
            print("     Cuisine: {} {}".format(recipe['cuisine'], "[OK]" if cuisine_ok else "[FAIL]"))
            print("     Time: {} min {}".format(recipe['cooking_time'], "[OK]" if time_ok else "[FAIL]"))
            print("     Overlap: {} ingredients {}".format(int(recipe['ingredient_overlap_count']), "[OK]" if overlap_ok else "[FAIL]"))
            print("     Score: {:.1%} {}".format(recipe['final_score'], "[OK]" if score_ok else "[FAIL]"))
    
    # TEST CASE 2: Italian cuisine with reasonable constraints
    print("\n" + "-"*70)
    ...
    if len(recommendations) > 0:
        print("\nRESULTS:")
        for idx, (_, recipe) in enumerate(recommendations.iterrows(), 1):
            print("\n  {}. {}".format(idx, recipe['recipe_name']))
            print("     Cuisine: {}".format(recipe['cuisine']))
            print("     Time: {} min".format(recipe['cooking_time']))
            print("     Ingredient Overlap: {}".format(int(recipe['ingredient_overlap_count'])))
            print("     Score: {:.1%}".format(recipe['final_score']))
    else:
    ...
    if len(recommendations) > 0:
        ...
        print("\n  All under 10 min: {}".format("[OK]" if all_under_10 else "[FAIL]"))
        print("\nSample results:")
    ...
    print("\nRequested: 10 | Received: {}".format(len(recommendations)))
    print("Duplicate recipe names: {} {}".format(duplicates, "[OK]" if duplicates == 0 else "[FAIL]"))
    
    print("\n" + "="*70)
    print("TEST SUMMARY")
    print("="*70)
    print("""
[OK] ENHANCEMENTS IMPLEMENTED:
...
[OK] EXPECTED IMPROVEMENTS:
   ...
   * Fallback message when constraints too strict
    ...
    print("\n[OK] All tests completed. Check results above.\n")
